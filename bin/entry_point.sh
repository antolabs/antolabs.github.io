...
start_jekyll() {
    manage_gemfile_lock
    chown -R jekyll:ruby /srv/jekyll
    su jekyll -c "bundle install" && \
    su jekyll -c "mkdir -p .jekyll-cache" && \
    su jekyll -c "bundle exec jekyll serve --watch --port=8080 --host=0.0.0.0 --livereload --verbose --trace --force_polling" &
}
